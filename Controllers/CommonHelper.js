...
var VINHelper = require('../Controllers/VINHelper');
//var BMWVINHelper = require('../Controllers/BMWVINHelper');
//var PorscheVINHelper = require('../Controllers/PorscheVINHelper');
...
exports.IncrementCount = function (apiKeyId, IP) {
...
function SaveLog(vinNumber, status, IP, apiKeyId) {
    var sqlQuery = "INSERT INTO vinlookuplog (VINNumber, IsSuccess, IPAddress, TimeStamp, APIKeyId) VALUES ("
        + "'" + vinNumber + "',"
        + status + ",'"
        + IP + "',"
    ...
    db.query(sqlQuery, function (err, result) {
        if (err) {
            errorLog('VIN - saveLog', new Date().toISOString().slice(0, 19).replace('T', ' '), IP, err.message);
        }
    })
}